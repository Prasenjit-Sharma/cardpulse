 ...
 * teals and steels, so a list of thirty reads as one page rather than a colour chart.
 */
export default function Avatar({ name, size = 44 }: { name: string; size?: number }) {
  const h = 165 + (hue(name || "?") % 60)   // teal through steel blue: the palette's own range
  return (
    <span
      className="avatar"
      style={{
