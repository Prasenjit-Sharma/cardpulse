...
### First run (Home with no cards)
The masthead carries a sample card (white, drop-shadowed) inside the camera's teal corner frame (Lock Teal, the "card locked" colour), with an indigo read line sweeping down it every 3.2 seconds, the screen's one motion, off with reduced motion. Below: "Scan your first card" at 22px, one line of what happens, one full-width 52px Scan key (the screen's one filled action), three reasons as flat rows with indigo tool wells (read in seconds; Hindi and regional scripts; several people from one photo), and a quiet link to start with your own digital card.

### Photo or logo
One upload, two treatments decided from the picture itself (`photoKind` in `src/lib/cardphoto.ts`): wider or taller than square, or white or transparent in all four corners, is a **logo**, drawn whole ("contain") on a white rounded plate like the QR's, the plate sized to the logo; anything else is a **face**, filling a circle. With a picture, each template reserves a slot 2.3 times its height (anchored right, or left on Split), so a wide logo stays readable; text keeps clear of the whole slot. Uploads get a white ground before JPEG encoding, so a transparent logo never turns black. Every template shows it (Bold places it bottom right, under the QR); stall mode shows it between the QR and the name.

### Save to phone
In the app, Save to phone opens the phone's own new-contact screen, filled in (Saved as name, numbers main first with their types, emails, website, address, note), and the user picks Google, Outlook or the phone there. No file is offered on the contact page. The public card page's button is "Save to phone": iPhone opens the contact card directly; Android hands the file to Contacts. Bulk vCard export stays in Settings as the file to import into Google Contacts, Outlook or iCloud.
