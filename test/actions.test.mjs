...
test('Share contact: vCard file plus readable text in one share', async () => {
  ...
  assert.equal(d.files[0].type, 'text/vcard'); assert.equal(d.files[0].name, 'Rajesh Shah.vcf'); assert.match(d.text, /\+91 98765 43210/); assert.match(d.text, /rajesh@abc\.com/)
})
...
test('files not shareable: Share falls back to text; Save to phone downloads (nothing else it can do) and says why', async () => {
  ...
  assert.equal(s2.result, 'downloaded'); assert.equal(b.calls.download[0][2], 'text/vcard')
})
